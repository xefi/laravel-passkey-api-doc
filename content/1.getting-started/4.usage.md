---
title: Usage
description: End-to-end example of passkey registration and authentication with Laravel Passkey API.
navigation:
  icon: i-lucide-play
---
...
The `POST /api/passkeys/login` endpoint does not require authentication. The shape of the response depends on the [`auth_action`](/getting-started/configuration#auth_action) configured in `config/passkey.php` — it can return a Sanctum token, a Passport token, or simply establish a web session.
::
